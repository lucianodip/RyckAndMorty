...
export const ItemListEpisodesPage1 = () => {
  ...
  const pedirEpisodes = (character1) =>{
    getEpisodes(character1).then((data) => 
      {data.episode.map((ep) =>{
  ...
  useEffect(() => {
    ...
    pedirEpisodes(character1);
    pedirEpisodesDetail(episodes);
    
  }, [character1]);

  //const {name} = episodesDetail;
  //console.log(name);
  //console.log(episodes);
  //console.log(episodesDetail);
  return (
    <div className='episodes'>
      <h1>Episodios personaje 1:</h1>
     {episodes.map((ep) =>(
        <li key={ep}>{ep}</li>
      ))} 
    </div>
